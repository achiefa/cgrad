...
static ValueData* value_create_internal(Tape* t, scalar_t data, const char* name,
                                        int requires_grad, const char* op,
                                        ValueData* child1, ValueData* child2) {
...
static void backward_add(ValueData* out) {
    ...
    if (out->children[0]) out->children[0]->grad += out->grad;
    if (out->children[1]) out->children[1]->grad += out->grad;
}

static void backward_mul(ValueData* out) {
    /* d/da (a * b) = b, d/db (a * b) = a */
    if (out->children[0]) out->children[0]->grad += out->cached_a * out->grad;
    if (out->children[1]) out->children[0]->grad += out->cached_b * out->grad;
}
